...
HTACCESS;
    ...
    public function runInstallation()
    {
        try {
            ...
            // Ensure a web entry point exists at document root (index.php shim)
            $ep = $this->ensureEntryPoint($baseDir);
            $result['docrootHasIndex'] = $ep['hadIndex'];
            if ($ep['created']) {
                $result['entryPointCreated'] = true;
                $result['entryPointTarget'] = $ep['target'];
            } else {
                $result['entryPointCreated'] = false;
            }
    ...
    /**
     * Create index.php shim at document root to route to actual app front controller if missing
     */
    private function ensureEntryPoint($baseDir)
    {
        $result = ['created' => false, 'target' => null, 'hadIndex' => false];
        $rootIndex = $baseDir . '/index.php';
        if (file_exists($rootIndex)) {
            $result['hadIndex'] = true;
            return $result;
        }

        // Candidate targets relative to root
        $candidates = [
            'public/index.php',
        ...
        foreach ($candidates as $rel) {
            if (file_exists($baseDir . '/' . $rel)) {
                $this->writeIndexShim($baseDir, $rel);
                $result['created'] = true;
                $result['target'] = $rel;
                return $result;
            }
        ...
        // Heuristic: look for single top-level directory with index.php or public/index.php
        $dirs = [];
        $dh = @opendir($baseDir);
        if ($dh) {
            while (($f = readdir($dh)) !== false) {
                if ($f === '.' || $f === '..') continue;
                $p = $baseDir . '/' . $f;
                if (is_dir($p) && !in_array($f, ['vendor','node_modules','.git'])) {
                    $dirs[] = $f;
                }
            }
            closedir($dh);
        }
        // Prefer a folder with an index
        foreach ($dirs as $d) {
            foreach (['index.php','public/index.php'] as $inner) {
                $rel = $d . '/' . $inner;
                if (file_exists($baseDir . '/' . $rel)) {
                    $this->writeIndexShim($baseDir, $rel);
                    $result['created'] = true;
    ...
    private function writeIndexShim($baseDir, $relTarget)
    {
        $shim = <<<PHP
<?php
// Auto-generated by installer to route to application front controller

\$target = __DIR__ . '/' . '$relTarget';
...
echo 'Entry point not found: ' . htmlspecialchars('$relTarget');
PHP;
        @file_put_contents($baseDir . '/index.php', $shim);
    }
